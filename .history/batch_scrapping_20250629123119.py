...
def parse_listing_fields(result):
    try:
        ...
        rating_elem = soup.find("p", class_="D_lm D_ln D_lr D_lu D_lx D_l_ D_bMk D_lI")
        seller_rating = float(rating_elem.contents[0].strip()) if rating_elem and rating_elem.contents else None
        years_elem = soup.find("span", string=re.compile(r"\d+\s+years\s+on\s+Carousell"))
        years = int(re.search(r"\d+", years_elem.text).group()) if years_elem else None
