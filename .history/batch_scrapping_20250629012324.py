...
def parse_listing_fields(result):
    try:
        ...
        seller_url = soup.findAll("span", class_="D_lm D_ln D_lq D_lt D_lx D_lz D_nT D_lH")[0]

        print(description, seller_name, seller_url, seller_rating_elem, review_elem, years_elem)
...
def update_db(listings):
    ...
    for listing in listings:
        cursor.execute('''
            UPDATE listings
            SET sold_status =?, sold_datetime =?, description =?, grading =?, seller_name =?, seller_url =?, seller_rating =?
            WHERE id = ?
        ''', (listing['sold_status'], listing['sold_datetime'], listing['description'], listing['grading'], listing['seller_name'], listing['seller_url'], listing['seller_rating'], listing['id']))
