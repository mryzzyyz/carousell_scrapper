...
def parse_listing_fields(result):
    try:
        ...
        # Find the seller's profile section
        seller_section = soup.find('p', string='Meet the seller').parent.find_next_siblings()
        print(f"Seller Section: {seller_section}")

        if seller_section:
            # Navigate to the parent container to find rating and years
            parent_div = seller_section.find_parent('div')

            # Extract the seller's rating
            rating_span = parent_div.find('span', string=lambda text: text and text.strip().replace('.', '').isdigit())
            seller_rating = rating_span.get_text(strip=True) if rating_span else 'Rating not found'

            # Extract the years on Carousell
            years_span = parent_div.find('span', string=lambda text: text and 'years' in text.lower())
            years_on_carousell = years_span.get_text(strip=True) if years_span else 'Years not found'

            # Print the extracted information
            print(f"Seller Rating: {seller_rating}")
            print(f"Years on Carousell: {years_on_carousell}")
