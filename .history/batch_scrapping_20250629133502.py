...
def load_url_ids_list(fetch_until):
    ...
    c = conn.cursor()
    c.execute(f"SELECT listing_url, id FROM listings WHERE DATE(datetime) >= DATE('now', '-{fetch_until} days')")
    rows = c.fetchall()
    conn.close()
    return rows
    ...
    try:
        ...
        for i in range(0, len(urls), max_concurrent):
            ...
            for url, result in zip(batch, results):
                ...
                print(ids[count], ":", url, "→", result.status_code)
...
def update_db(listings):
    ...
    for listing in listings:
        ...
        cursor.execute("SELECT EXISTS(SELECT 1 FROM listings WHERE id = ?)", (listing['id'],))
        print("Exists in DB?", cursor.fetchone()[0], "→", listing['id'])
