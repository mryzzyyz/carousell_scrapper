...
def parse_listing_fields(result):
    try:
        ...
        seller_name_elem = soup.find("p", class_="D_lm D_ln D_lr D_lu D_lx D_l_ D_bMk D_lH")
        seller_name = seller_name_elem.text.strip() if seller_name_elem else ""
        seller_rating_elem = soup.find("span", class_="D_lm D_ln D_lr D_lu D_lw D_l_ D_bMw D_bMx D_lI")
        print(seller_rating_elem)

        review_elem = soup.find("p", string=lambda t: "review" in t.lower())  # finds first <p> with "review"
        
        years_elem = soup.find("span", class_="D_lm D_ln D_lr D_lu D_lw D_l_ D_bMw D_lI")
        
        seller_url_elem = soup.find("a", class_="D_biE D_biI D_lk")
        if seller_url_elem is not None:
            print("✅ Seller URL Element HTML:\n", seller_url_elem)
            print("✅ href:", seller_url_elem.get("href"))
            seller_url = "https://www.carousell.sg" + seller_url_elem.get("href", "")
        else:
            print("❌ seller_url_elem not found. Dumping HTML snippet...")
            print(soup.prettify()[:2000])  # limit to first 2000 chars to avoid flooding
            seller_url = ""
        ...
        return {
            ...
            "years_on_carousell": int(years_elem.text.strip()) if years_elem else ""
        }
...
def update_db(listings):
    ...
    for listing in listings:
        cursor.execute('''
            UPDATE listings
            SET sold_status =?, sold_datetime =?, description =?, grading =?, seller_name =?, seller_url =?, seller_rating =?, review_count =?, years_on_carousell =?
            WHERE id = ?
        ''', (listing['sold_status'], listing['sold_datetime'], listing['description'], listing['grading'], listing['seller_name'], listing['seller_url'], listing['seller_rating'], listing['review_count'], listing['years_on_carousell'], listing['id']))
