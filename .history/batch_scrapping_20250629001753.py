...
def parse_listing_fields(result):
    soup = BeautifulSoup(result.data.get("page_content", ""), "html.parser")
        ...
        for i in range(0, len(urls), max_concurrent):
            ...
            for i, result in enumerate(results):
                if isinstance(result, Exception):
                    print(f"[❌] URL {i} failed with error: {result}")
                elif hasattr(result, "data") and isinstance(result.data, dict):
                    print(f"[✅] Keys for result {i}: {result.data.keys()}")
                else:
                    print(f"[⚠️] Unexpected structure for result {i}: {type(result)}")
